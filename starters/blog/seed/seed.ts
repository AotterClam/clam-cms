 ...
 * `wrangler d1 execute clam-blog-local --local --file=/dev/stdin` (or
 ...
 *   pnpm seed | wrangler d1 execute clam-blog-local --local --file=/dev/stdin
 *
 * Or simpler:
 *   pnpm seed > .seed.sql && wrangler d1 execute clam-blog-local --local --file=.seed.sql
 */
    ...
    translations: [
      {
        locale: "en",
        title: "Hello, world",
        body: "This is the first post on the Clam blog. Localized content rendered from KV; the body is plain markdown for v0.1.0 (real markdown rendering arrives in starter v2).",
      },
      {
        locale: "zh-TW",
        title: "你好，世界",
        body: "這是 Clam blog 的第一篇文章。內容從 KV 渲染、依語系切版；v0.1.0 的 body 暫以純文字呈現，正式 markdown 渲染留給 starter v2。",
      },
    ],
  },
  {
