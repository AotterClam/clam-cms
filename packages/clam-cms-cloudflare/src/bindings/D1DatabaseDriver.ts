 ...
 * `_clam_migrations` table so subsequent boots are idempotent.
 *
 ...
 * includes the migration's SQL plus the `_clam_migrations` row insert,
...
class D1Migrations implements MigrationRunner {
  ...
  async runAll(migrations: ReadonlyArray<Migration>): Promise<void> {
    await this.db
      .prepare(
        `CREATE TABLE IF NOT EXISTS _clam_migrations (id TEXT PRIMARY KEY, applied_at INTEGER NOT NULL)`,
      )
      .run();
    const applied = await this.db
      .prepare(`SELECT id FROM _clam_migrations`)
      .all<{ id: string }>();
    ...
    for (const m of migrations) {
      ...
      ops.push(
        this.db
          .prepare(`INSERT INTO _clam_migrations (id, applied_at) VALUES (?, ?)`)
          .bind(m.id, Date.now()),
      );
