import type { OAuthIdentity, OAuthVerifier } from "@aotterclam/clam-cms-runtime";
 ...
 *
 * The constructor REQUIRES `env.CLAM_ALLOW_STUB_OAUTH === "1"` so a
...
export class StubOAuthVerifier implements OAuthVerifier {
  constructor(env: { readonly CLAM_ALLOW_STUB_OAUTH?: string }) {
    if (env.CLAM_ALLOW_STUB_OAUTH !== "1") {
      throw new Error(
        "StubOAuthVerifier refuses to run without env.CLAM_ALLOW_STUB_OAUTH='1'. " +
