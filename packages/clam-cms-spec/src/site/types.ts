 ...
 *
 * Lives in `clam-cms-spec` (not `-runtime`) because the spec validator
 ...
 * `clam-cms-runtime` — they go through the `DatabasePort`.
 *
 ...
 * happens in `clam-cms-runtime` via `loadSiteConfig(env)` — spec
