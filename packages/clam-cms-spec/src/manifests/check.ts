...
import { CLAM_BIND_VALUES } from "./grammar.js";
...
/**
 * Loop 1 (`clam-cms validate`) — pure manifest + handler-source
 * check. No D1, no runtime, no network. ADR-0007 / authoring-contract.md.
 *
 * The structural parser (manifests/parse.ts) catches single-manifest
 * envelope / shape / DRAFT-key errors. This module catches everything
...
export interface ValidateInput {
   ...
   *  check (ADR-0010). When omitted, the SCHEMA_LOCALIZED_REQUIRES_SITE_LOCALES
   *  check is skipped — the CLI typically can't reach the runtime D1
   *  to read site_config. The boot validator always supplies this. */
...
export function check(input: ValidateInput): ValidateResult {
  const diags: Diagnostic[] = [];
  const partitioned = partitionManifests(input.manifests);
  const schemasByName = byName(partitioned.schemas);
  const proceduresByName = byName(partitioned.procedures);

  diags.push(...checkDuplicates("Schema", partitioned.schemas, input.filePaths));
  diags.push(...checkDuplicates("View", partitioned.views, input.filePaths));
  diags.push(...checkDuplicates("Procedure", partitioned.procedures, input.filePaths));
  diags.push(...checkDuplicates("Trigger", partitioned.triggers, input.filePaths));

  for (const s of partitioned.schemas) {
    diags.push(...checkSchemaInternals(s, input.filePaths));
  }
  ...
  diags.push(
    ...checkLocaleAndTranslates({
      ...
      phase: "validate",
      siteLocales: input.siteLocales,
      filePaths: input.filePaths,
    }),
  );

  for (const v of partitioned.views) {
    diags.push(...checkViewRefs(v, schemasByName, input.filePaths));
  }

  for (const p of partitioned.procedures) {
    diags.push(...checkProcedureAuth(p, input.filePaths));
    diags.push(...checkBuiltinHandler(p, schemasByName, input.filePaths));
  }

  diags.push(...checkTriggerRefs(partitioned.triggers, proceduresByName, input.filePaths, schemasByName));

  if (input.handlerSource !== undefined) {
    diags.push(...checkHandlerRefsInSource(partitioned.procedures, input.handlerSource, input.filePaths));
  }
...
function checkViewRefs(
  ...
  // Reserved metadata fields — kept on the entries table column-shape
  // even after ADR-0010 dropped `locale` (which moved into data and
  // is now declared per-Schema like any other field).
...
function checkBuiltinHandler(
  ...
  // op: archive is editorial-only (ADR-0011 / ADR-0014). Inline the
  // lifecycle resolution here — we need only the Schema.spec.lifecycle
  // default, not the full state-machine helpers (those live in the
  // runtime package and would pull non-spec deps).
