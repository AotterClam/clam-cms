...
export type JsonSchema = {
  ...
  readonly "x-clam-ref"?: string;
  /** Custom: hint for MCP tool / agent prompt context. */
  readonly "x-mcp-hint"?: string;
  readonly [key: string]: unknown;
};

export const CLAM_REF_KEYWORD = "x-clam-ref" as const;
export const MCP_HINT_KEYWORD = "x-mcp-hint" as const;
export const CLAM_BIND_KEYWORD = "x-clam-bind" as const;
...
export interface SchemaManifestSpec {
   ...
   *  v0.1 property extensions: `x-clam-bind`, `x-clam-ref`, `x-mcp-hint`. */
...
export const CLAM_BIND_VALUES = ["ctx.user", "ctx.staff", "now"] as const;
export type ClamBindValue = (typeof CLAM_BIND_VALUES)[number];
