...
describe("InvokeBuiltinUseCase — create", () => {
  it("projects input ∩ Schema.properties and stamps x-clam-bind", async () => {
    const h = harness();
  ...
  it("server-stamping wins over caller-supplied x-clam-bind values", async () => {
    const h = harness();
    const result = await h.invoke.execute({
      procedure: createPostFullInput,
      input: {
  ...
  it("anonymous ctx → x-clam-bind: ctx.user stamps null", async () => {
    const h = harness();
    const result = await h.invoke.execute({
      procedure: createPostFullInput,
      input: { title: "anon" },
