...
> If you're an AI working in a project that consumes `@aotterclam/clam-cms-*`,
...
extensions. Three are part of `cms.clam.ai/v1`:

#### `x-clam-bind: <value>` — server-stamped fields
...
- Used by `requires.auth.owns:` predicate (DRAFT): the predicate
  compares a row's `x-clam-bind: ctx.user` field to the current
...
arbitrary expressions (`x-clam-bind: ${request.headers["x-team"]}`),
...
#### `x-clam-ref: <other-schema-name>` — cross-Schema reference
...
`@aotterclam/clam-cms-spec` as the `CLAM_REF_KEYWORD` constant; SDK
...
owner declarations, `x-clam-bind` enum extension) is **DRAFT** — see
appendix.
...
**v0.1.0 `handler.kind`** is `ref` only. `builtin` (a thin shortcut
for trivial CRUD-shaped Procedures) is **v0.1.x** (committed roadmap)
— full spec in the Roadmap appendix § "v0.1.x committed."
...
**v0.1.0 `Trigger.source.kind`** is `http` only.

- `lifecycle` is **v0.1.x** (committed roadmap) — full spec in the
  Roadmap appendix § "v0.1.x committed."
...
- **Runtime engine** = zod (v3). The `@aotterclam/clam-cms-spec`
...
(`@aotterclam/clam-cms-spec`); different collections coexist on the
...
| Path operators | `->`, `->>` only | `->`, `->>`, `#>`, `#>>`, `@>`, `?`, `?|`, `?&` |
| `DEFAULT now()` | not at column level — SDK-stamped via `x-clam-bind: now` | native column default |
...
- **Mid-scale (10k – 100k entries)**: list/filter Views on
  ...
  `x-clam-ref` auto-lift. Both are reserved in the future-grammar
  appendix.
...
remain valid `cms.clam.ai/v1` deployments.
...
(`@aotterclam/clam-cms-netlify`) is a README stub for v0.2; its
...
1. **v0.1.x committed** — already on the patch-release roadmap. Spec
   is documented; implementation lands within the v0.1 series. Boot
   validator in v0.1.0 rejects these keys with a code naming the
   feature (e.g. `LIFECYCLE_NOT_IN_V010`) and a message pointing at
   the v0.1.x roadmap.
2. **DRAFT (v0.2+)** — speculative, gated by concrete consumer
...
- **`x-clam-ref` auto-lift to virtual column** — when a property
  carrying `x-clam-ref: <other-schema>` is referenced in any registered
