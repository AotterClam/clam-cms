...
You're installing a clam-cms site. The composed URL inlined this brief plus the per-archetype hint — archetype-specific register cues are in the same document.
...
Before running `create-clam-cms`, you need: archetype, brand, locales, GitHub identity. Below that, ask one more concrete question. Above, synthesize a one-paragraph draft and propose it back for confirmation.
...
A permitted modification turn after `create-clam-cms` returns and before provision. Small concrete edits to match what the user actually said.
