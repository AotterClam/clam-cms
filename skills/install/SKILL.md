...
|---|---|---|---|
| `publication` | **available** (`AotterClam/clam-cms-starter-publication`) | Owner-published content — landing pages, articles, docs-lite, project updates, basic contact form, simple public widgets / calculators. | Inventory / order workflows; lead-management pipelines; member-created content; private/paid creator content. |
...
Use public GitHub repos only as starter template sources. Do not build the `clam-cms` SDK monorepo inside the consumer project.

For `publication`, download a tarball of the standalone starter repo and strip the top-level archive directory:

```bash
curl -L "https://codeload.github.com/AotterClam/clam-cms-starter-publication/tar.gz/<template_ref>" -o /tmp/clam-cms-starter-publication.tgz
tar -xzf /tmp/clam-cms-starter-publication.tgz --strip-components=1
```

This intentionally avoids leaving `origin` pointed at `AotterClam/clam-cms-starter-publication`. After extraction, initialize a fresh user-owned repo without adding a remote:
...
```bash
mkdir -p .clam-cms-template
curl -L "https://codeload.github.com/AotterClam/clam-cms/tar.gz/<template_ref>" -o /tmp/clam-cms-template.tgz
tar -xzf /tmp/clam-cms-template.tgz -C .clam-cms-template --strip-components=1
```
...
- Rewrites copied starter dependencies to `@aotterclam/*@<clam_cms_version>`.
...
| Symptom | Cause | Fix |
|---|---|---|
| Starter tarball download fails | Network issue, bad `template_ref`, or GitHub outage | Verify `template_ref` exists on `AotterClam/clam-cms-starter-publication`; retry the `codeload.github.com` URL or download the tagged source zip manually. |
| `git remote -v` points at `AotterClam/clam-cms-starter-publication` | The project was cloned instead of extracted from tarball | Remove the remote before any push: `git remote remove origin`, then set `origin` only to the user's own repo. |
| `pnpm run setup:site` is missing | Starter was copied from an older template ref | Re-copy from `main` or a newer release tag. |
| `pnpm install` cannot resolve `workspace:*` | The consumer project was copied from the monorepo mirror and `setup:site` was skipped before install | Run `pnpm run setup:site -- ... --clam-cms-version "<version>"`, then `pnpm install` again. Publication installs from the standalone starter repo should not hit this. |
