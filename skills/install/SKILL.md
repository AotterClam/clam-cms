...
description: Bootstrap a new clam-cms consumer project from the reference starter. Use when the user says "I want a blog / a docs site / a marketing site backed by an MCP-native CMS" and there isn't already a clam-cms project in the working directory.
when_to_invoke: |
  The user is starting from scratch. Indicators: empty repo, generic "I want a website", no existing manifests/ directory, no @aotterclam/* deps in package.json.
applies_to: clam-cms@v0.1.0
---

# Install a clam-cms project

You (the agent) are bootstrapping a new clam-cms consumer for the user. Read this entire file before touching the filesystem. The reference starter is `starters/blog` in <https://github.com/AotterClam/clam-cms>; it deploys to Cloudflare Workers + D1 + KV.
...
git clone --depth 1 https://github.com/AotterClam/clam-cms.git .clam-tmp
cp -R .clam-tmp/starters/blog/. .
rm -rf .clam-tmp

# Or, when published as a template:
# pnpm dlx @aotterclam/create-blog <name>
```

### 2. Configure for the user's site

Edit `src/clamConfig.ts`:
...
```bash
pnpm install
pnpm validate          # `clam-cms validate`. Exits 0 with 0 errors.
...
pnpm emit-types        # → clam-types.d.ts (typed handler signatures)
```

Add `clam-types.d.ts` to `tsconfig.json` `include` so handler files get typed.
...
| `validate` warns `SCHEMA_LOCALIZED_REQUIRES_SITE_LOCALES`         | A localized Schema has no matching `siteDefaults.locales`            | Set `locales: ["en", ...]` in `clamConfig.ts`. CLI can't read site_config; warning is intentional heads-up.   |
...
- Don't commit `openapi.json` / `clam-types.d.ts` to the upstream starter; they're consumer-specific outputs.
